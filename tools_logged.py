"""
Marine Corps Operations Assistant (MCOA) - Mock Tools with Logging
These are simulated tools with visual logging for demonstration purposes
"""
...
def log_tool_call(tool_type: str, color: str = Colors.CYAN):
    """Decorator to log tool calls with visual indicators."""
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            # Print tool call header
            print(f"\n{color}{'='*60}{Colors.END}")
            print(f"{color}{Colors.BOLD}🔧 TOOL CALL: {func.__name__}{Colors.END}")
            print(f"{color}📍 Section: {tool_type}{Colors.END}")
            print(f"{color}⏰ Time: {datetime.now().strftime('%H:%M:%S')}{Colors.END}")
            
            # Print parameters
            if args or kwargs:
                print(f"{color}📊 Parameters:{Colors.END}")
                for i, arg in enumerate(args):
                    print(f"  {color}• arg[{i}]: {arg}{Colors.END}")
            ...
            print(f"{Colors.YELLOW}⚙️  Processing...{Colors.END}", end='', flush=True)
            ...
            # Print completion
            print(f"\r{Colors.GREEN}✅ Complete!     {Colors.END}")
            
            # Show brief result preview
            if isinstance(result, BaseModel):
                print(f"{Colors.GREEN}📤 Result: {result.__class__.__name__} object{Colors.END}")
            elif isinstance(result, dict):
                print(f"{Colors.GREEN}📤 Result: Dict with {len(result)} keys{Colors.END}")
            elif isinstance(result, list):
                print(f"{Colors.GREEN}📤 Result: List with {len(result)} items{Colors.END}")
            else:
                result_str = str(result)[:100]
                print(f"{Colors.GREEN}📤 Result preview: {result_str}...{Colors.END}" if len(str(result)) > 100 else f"{Colors.GREEN}📤 Result: {result_str}{Colors.END}")
...
@function_tool
@log_tool_call("S-4 LOGISTICS", Colors.BLUE)
...
@function_tool
@log_tool_call("S-2 INTELLIGENCE", Colors.MAGENTA)
...
@function_tool
@log_tool_call("S-3 OPERATIONS", Colors.YELLOW)
